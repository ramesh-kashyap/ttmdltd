...
class UserController extends Controller
{
    ...
    public function changeStatus(Request $request)
    {
        ...
        if ($task) {
            $task->active_status = $task->active_status === 'Active' ? 'Inactive' : 'Active';
            $task->save();
    
            return response()->json(['success' => true]);
        }
    
        $this->data['page'] = 'admin.users.task-list';
        return $this->admin_dashboard();
    }
    ...
    public function task_list(Request $request)
    {
     ...
     $this->data['search'] = $search;
     $this->data['page'] = 'admin.task.task-list';
     return $this->admin_dashboard();
...
public function add_bonus()
    ...
    public function add_task()
    {
     ...
     $this->data['page'] = 'admin.task.add-task';
     return $this->admin_dashboard();
...
public function add_bonus_post(Request $request)
        ...
        $countryCodeMap = [
          ...
          "Zimbabwe"=> "zw"
            ...
            $validation =  Validator::make($request->all(), [
              ...
              'company_image' => ['required', 'image', 'mimes:jpg,jpeg,png,webp', 'max:2048'],
              'country_name' => ['required', 'string', 'max:255', function ($attribute, $value, $fail) use ($countryCodeMap) {
                  if (!array_key_exists($value, $countryCodeMap)) {
                      // Find closest matching country
                      $closest = null;
                      $shortest = -1;
          
                      foreach (array_keys($countryCodeMap) as $country) {
                          $lev = levenshtein(strtolower($value), strtolower($country));
                          if ($lev == 0) {
                              $closest = $country;
                              $shortest = 0;
                              break;
                      ...
                      $fail("The selected country \"$value\" is invalid." . $suggestion);
                  }
              }],
            ...
            if ($request->hasFile('company_image')) {
              ...
              // Move image to public/company_images
              $image->move(public_path('company_image'), $filename);
          
              // Save path for DB
              $imagePath = '' . $filename;
          } else {
              $imagePath = null; // Or default path
          }
                ...
                $notify[] = ['success', 'Task has been added successfully '];
               return redirect()->back()->withNotify($notify);
  ...
  public function available_balance()
   ...
   public function block_users(Request $request)
    ...
    public function add_notification_post(Request $request)
    {
      ...
      try{
            ...
            if($validation->fails()) {
                ...
                return Redirect::back()->withErrors($validation->getMessageBag()->first())->withInput();
                  ...
                  $income = Notification::Create($data);   
    
                $notify[] = ['success', 'Notification  Added successfully'];
               return redirect()->back()->withNotify($notify);
    ...
    public function add_notification()
    {
     ...
     $this->data['page'] = 'admin.users.notification';
     return $this->admin_dashboard();
